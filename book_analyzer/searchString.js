...
function loadBook(fileName, displayName) {
  ...
  xhrOBJ.onreadystatechange = function () {
    if (xhrOBJ.readyState == 4 && xhrOBJ.status == 200) {
      currentBook = xhrOBJ.responseText;
      getDocsStats(currentBook);

      var elmnt = document.getElementById("fileContent");

      //remove line breaks and carriage returns and replace wiht a br element
      currentBook = currentBook.replace(/(?:\r\n|\r|\n)/g, "<br>");

      elmnt.innerHTML = currentBook;
      elmnt.scrollTop = 0;
    }
  };
}
...
function getDocsStats(fileContent) {
  ...
  var bottom5Words = wordList.slice(-6, wordList.length);

  // writing values to page
  ULTemplate(top5Words, document.getElementById("mostUsed"));
  ULTemplate(bottom5Words, document.getElementById("leastUsed"));
...
// list fo stop words we don't want in stats
